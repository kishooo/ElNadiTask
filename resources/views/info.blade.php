...
<table id="myTable">
  ...
  <th style="width:20%;">id</th>
    ...
    @foreach($types as $type)
        <th style="width:40%;">{{$type->type}}</th>
    @endforeach
    @endif
  ...
  <tr id="{{$student->studentId}}">
    ...
    @foreach($gradeStudents as $gradeStudent)
        @if($student->studentId == $gradeStudent->SID)
        <td>{{$gradeStudent->StudentGrade}}</td>
        @php
            $sum+= $gradeStudent->StudentGrade
        @endphp
        @endif
    @endforeach
    <td>total is {{$sum}}</td>
    ...
    <td><button  class="remove button button4" data-product="{{$student->studentId}}" data-course="{{$courseId}}" type="button" id="remove">remove</button></td>
...
$(document).ready(function(){
        $('.remove').click(function (event) {
          ...
          $.ajax({
            async:'false',
            type:'Post',
            url:"{{ route('removeStudentCourse') }}",
            data:{ productId:productId,courseId:courseId,"_token": "{{ csrf_token() }}"},
